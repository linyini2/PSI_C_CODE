...
// const uint32_t inter = uint32_t(_two31 / 2 * 2);
...
void PolyMul(uint32_t *h_a, int32_t *h_trlwekey, uint32_t *product, Params128 p128)
{
    int M = p128.N / 2;
    cufftHandle plan;
    ...
    // process mul
    cuDoubleComplex *h_Comp_product = (cuDoubleComplex *)malloc(M * sizeof(cuDoubleComplex));
...
void Test()
{
    Params128 p128 = Params128(1024, 630, 2, 10, pow(2.0, -15.4), pow(2.0, -28), 2);
    // generate message
